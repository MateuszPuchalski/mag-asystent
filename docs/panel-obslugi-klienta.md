...
| Alarm „to stoi za długo" | **działa** od 0.364.0 | `alarmyWymiany()`, `GET /api/biuro/alarm-wymiany`, plakietka w pasku stanu; próg to `p90` spraw domkniętych w TYM kanale, osobno dla każdego z pięciu |
| Notatka do dostawy w mierze i alarmie | **działa** od 0.364.0 | wyłączona w 0.361.0 przez uzasadnienie wzięte z `conversation_event`; `delivery_note` ma jedną odpowiedź, a brak odpowiedzi wstrzymuje domknięcie dostawy |
