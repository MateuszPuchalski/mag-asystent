...
import { mssqlWrite } from "../db/mssql.js";
 ...
 *  • set_location → UPDATE tw__Towar SET tw_Lokalizacja=@v WHERE tw_Id=@id
...
export class SqlSferaAdapter implements SferaAdapter {
  async applySetLocation(twId: number, newValue: string): Promise<void> {
    ...
    const res = await pool
      ...
      .query("UPDATE tw__Towar SET tw_Lokalizacja = @v WHERE tw_Id = @id");
