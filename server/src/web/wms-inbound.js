...
window.WmsInbound = (h) => {
  ...
  async function render() {
    window.document
      ...
      .classList.remove("wms-inbound-focus");
    line = null;
    if (selected) {
      ...
      target().innerHTML = `<div class="wms-toolbar"><button data-inbound="list">← Przyjęcia</button><button data-inbound="refresh">Odśwież</button></div>
        <section class="wms-surface"><h2>${html(document.reference)}</h2><p>${html(document.supplier)} · ${document.closed_at ? "Zamknięte" : `${remaining} szt. do rozliczenia`}</p>
        ${document.closed_at ? `<p>Zamknięto: ${html(date(document.closed_at))}. ${html(document.close_reason)}</p>` : `<form id="wms-inbound-scan" class="wms-toolbar">${field("barcode", "Zeskanuj towar / SKU")}<button>OTWÓRZ POZYCJĘ</button></form>`}
        <div id="wms-inbound-task"></div><details><summary>Pozycje dokumentu (${document.lines.length})</summary><p class="wms-help">Pokazano pierwsze 50 pozycji. Skan wyszukuje w całym dokumencie.</p><div class="wms-scroll"><table class="wms-lines"><thead><tr><th>SKU / część</th><th>Oczekiwane</th><th>Odłożone</th><th>Uszkodzone</th></tr></thead><tbody>${document.lines
          .slice(0, 50)
          .map(
            (l) =>
        ...
        <details class="wms-surface"><summary>Ostatnie odłożenia</summary>${document.putaways.map((p) => `<p>${html(p.sku)} · ${p.quantity} szt. → <strong>${html(p.bin)}</strong> · ${p.disposition === "damaged" ? "Uszkodzone" : "Pełnowartościowe"} · ${html(date(p.created_at))}${p.reason ? ` · ${html(p.reason)}` : ""}${p.reversed_at ? ` · Wycofano: ${html(p.reversal_reason)}` : !document.closed_at && h.office() ? ` <button data-inbound-reverse="${p.id}">Korekta odłożenia</button>` : ""}</p>`).join("") || "<p>Brak odłożeń. Sam dokument nie dodaje zapasu.</p>"}<div id="wms-inbound-correction"></div></details>
    ...
    target().innerHTML = `<form id="wms-inbound-filter" class="wms-toolbar">${field("q", "Dokument lub dostawca", "text", q)}<label>Zakres<select name="closed"><option value="0">Otwarte</option><option value="1" ${closed === "1" ? "selected" : ""}>Zamknięte</option></select></label><button>SZUKAJ</button></form>
      <section class="wms-surface"><h2>Dostawy do odłożenia</h2><p class="wms-help">Skan towaru → ilość → skan półki. Potwierdzenie odłożenia od razu aktualizuje zapas. Nie przyjmuj tego dokumentu ponownie w Zapasach.</p>
      <div class="wms-scroll"><table class="wms-lines"><thead><tr><th>Dokument / dostawca</th><th>Odłożone / oczekiwane</th><th>Brakujące</th><th>Utworzono</th></tr></thead><tbody>${data.rows.map((d) => `<tr><td><button data-inbound-open="${d.id}">${html(d.reference)}</button><br>${html(d.supplier)}</td><td>${d.received} / ${d.expected}${d.damaged ? `<br>${d.damaged} uszkodzonych` : ""}</td><td>${d.remaining}</td><td>${html(date(d.created_at))}</td></tr>`).join("") || '<tr><td colspan="4">Brak dokumentów w tym zakresie.</td></tr>'}</tbody></table></div><div class="wms-toolbar"><button data-inbound="prev" ${offset === 0 ? "disabled" : ""}>Poprzednie</button><span>${data.total} dokumentów</span><button data-inbound="next" ${offset + 50 >= data.total ? "disabled" : ""}>Następne</button></div></section>
  ...
  function openLine(id, scanned = "") {
    ...
    window.document.getElementById("wms-inbound-task").innerHTML =
      `<section class="wms-surface"><div class="wms-inbound-product">${h.photo(line)}<div><h3>${html(line.sku)}</h3><p>${html(line.name)}</p><p>Odłożono ${line.received} / ${line.expected} szt. · Pozostało ${Math.max(0, line.expected - line.received)}</p><p class="wms-help">${line.bins.length ? `Istniejące miejsca: ${line.bins.map((b) => `${html(b.bin)} (${b.mode === "pick" ? "kompletacja" : "zaplecze"}, ${b.on_hand} szt.)`).join(", ")}` : "Brak dotychczasowej półki. Zeskanuj zarejestrowaną lokalizację po odłożeniu."}</p></div></div>
      <form id="wms-inbound-putaway" class="wms-form">${field("barcode", "Kod towaru / SKU", "text", scanned)}<div class="wms-fields">${field("quantity", "Odłożona ilość", "number", 1, 'min="1" max="1000000" step="1"')}<label>Stan towaru<select name="disposition"><option value="good">Pełnowartościowy</option><option value="damaged">Uszkodzony → kwarantanna</option></select></label></div>${field("bin", "Zeskanuj lokalizację odłożenia")}<details><summary>Uszkodzenie / nadwyżka — uzasadnienie</summary><label>Co stwierdzono?<textarea name="reason" maxlength="500"></textarea></label><p class="wms-help">Uszkodzenie wymaga opisu i kwarantanny. Nadwyżkę zatwierdza biuro z uzasadnieniem.</p></details><button class="primary">POTWIERDŹ ODŁOŻENIE</button></form></section>`;
