...
window.WmsCarts = (h) => {
  ...
  function drawRun() {
    ...
    panel(`<div class="wms-actions"><button data-cart-action="list">WSZYSTKIE WÓZKI</button><button data-cart-action="reload">ODŚWIEŻ TRASĘ</button></div><h2>${html(run.cart_code)} · ${run.assigned_count}/${run.capacity} pozycji</h2>
      ${
        task && !run.closed_at
          ? `<div class="wms-cart-next"><div><span class="wms-eyebrow">POZYCJA SKRZYNKI</span><strong class="wms-cart-position">${task.position}</strong><span>${html(task.tote)}</span></div><div><span class="wms-eyebrow">LOKALIZACJA</span><strong class="wms-location">${html(task.bin)}</strong><strong>${html(task.sku)}</strong><p>${html(task.name)}</p><p>Łącznie na przystanku: <strong>${task.stop_quantity} szt.</strong><br>Do tej skrzynki: <strong>${task.remaining} szt.</strong></p></div></div>
      <form id="wms-cart-pick" class="wms-form">${verified ? `<input type="hidden" name="bin" value="${html(verifiedStop.bin)}"><input type="hidden" name="barcode" value="${html(verifiedStop.barcode)}"><p class="wms-help">Lokalizacja i towar potwierdzone na tym przystanku. Zeskanuj skrzynkę ${task.position}.</p>` : `${field("bin", "1. Lokalizacja", "text", "", 'autocomplete="off"')}${field("barcode", "2. Towar", "text", "", 'autocomplete="off"')}`}
        <div class="wms-fields">${field("quantity", "Sztuki do tej skrzynki", "number", task.remaining, `min="1" max="${task.remaining}"`)}${field("tote", `Skrzynka ${task.position}`, "text", "", 'autocomplete="off"')}</div><button class="primary">POTWIERDŹ ODŁOŻENIE</button></form>
