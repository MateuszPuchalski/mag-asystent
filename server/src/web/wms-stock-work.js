...
window.WmsStockWork = (h) => {
  ...
  async function render() {
    ...
    target().innerHTML = `<section class="wms-surface"><h2>Zadania zapasu</h2><p>Uzupełnienia wynikają z minimów półek oraz brakujących rezerwacji otwartych zamówień. Listy pokazują do 100 pozycji.</p>
  ...
  async function click(button) {
    ...
    if (button.dataset.stockworkClaim !== undefined) {
      ...
      const result = await mutate("/api/wms/replenishments", {
        ...
        source: p.source,
        target: p.target,
        quantity: Math.min(p.quantity, p.source_available),
