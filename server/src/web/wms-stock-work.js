...
window.WmsStockWork = (h) => {
  ...
  async function submit(form, values) {
    ...
    if (result) {
      ...
      if (result.completed)
        h.message(
          `Przesunięto ${result.quantity} szt.${result.shortage ? ` Brak ${result.shortage} szt. Źródło czeka na przeliczenie.` : ""}`,
        );
      if (result.recount)
        h.message(
