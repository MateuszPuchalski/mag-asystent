...
window.WmsPacking = ({ html, field, form, recovery }) => {
  ...
  function corrections(o) {
    return `${recovery(o)}<details class="wms-parcel-corrections"><summary>Popraw zawartość lub kontrolę</summary><p>Przełóż sprawdzone sztuki fizycznie do wskazanej paczki.</p>${form("pack-move", `${field("barcode", "Zeskanuj przekładany towar")}${field("quantity", "Sztuki", "number", 1, 'min="1"')}<div class="wms-fields">${field("fromParcel", "Z paczki", "number", 1, 'min="1" max="20"')}${field("toParcel", "Do paczki", "number", 2, 'min="1" max="20"')}</div>`, "POTWIERDŹ PRZEŁOŻENIE")}<details><summary>Cofnij kontrolę wybranych sztuk</summary><p>Wybierz paczkę i zeskanuj część wymagającą ponownego sprawdzenia. Towar pozostaje przy stanowisku; pozostałe potwierdzenia zostają. Przy braku lub uszkodzeniu wstrzymaj zamówienie.</p>${form("pack-unpack", `${field("barcode", "Zeskanuj część do ponownej kontroli")}${field("fromParcel", "Z paczki", "number", 1, 'min="1" max="20"')}${field("quantity", "Sztuki do ponownej kontroli", "number", "", 'min="1" max="1000000"')}${field("reason", "Powód korekty")}`, "COFNIJ KONTROLĘ TYCH SZTUK")}</details><details><summary>Sprawdź całe zamówienie od początku</summary><p>Wyjmij i ponownie zeskanuj zawartość. Zebrany zapas pozostaje przy stanowisku.</p>${form("pack-reset", field("reason", "Powód ponownej kontroli"), "ROZPOCZNIJ KONTROLĘ OD NOWA")}</details></details>`;
  }
  function labels(o) {
    const ns = numbers(o);
