...
export function Product() {
  ...
  const chipMenu = useStore((s) => s.chipMenu);
  const queue = useStore((s) => s.queue);
  if (!p) return null;

  const avail = p.mag - p.rez;
  const locStr = p.locs.join(" ");
  const hasPendingMM = queue.some(
    (t) => t.pid === p.id && t.status !== "done" && t.status !== "error" && t.type !== "set_location"
  );
  ...
  return (
    <div className="no-scrollbar flex flex-1 flex-col gap-3 overflow-y-auto p-3">
      <div>
        ...
        {p.desc && (
          <div className="mt-1.5 line-clamp-2 text-[11.5px] leading-snug text-ink-mute">
            {p.desc}
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded-lg border bg-card p-3">
          <div className="text-[10px] font-bold tracking-[0.12em] text-ink-mute">
            MAG · DOSTĘPNE
          </div>
          <div className="font-cond text-3xl font-extrabold leading-tight">{avail}</div>
          <div className="text-[11px] text-ink-soft">rez. {p.rez} · razem {p.mag}</div>
        </div>
        <div className={cn("rounded-lg border p-3", p.mgp > 0 ? "border-amber-line bg-amber-bg" : "bg-card")}>
          <div className="text-[10px] font-bold tracking-[0.12em] text-ink-mute">
            MGP · STREFA PRZYJĘĆ
          </div>
          <div className={cn("font-cond text-3xl font-extrabold leading-tight", p.mgp > 0 && "text-amber-ink")}>
            {p.mgp}
          </div>
          <div className="text-[11px] text-ink-soft">
            {p.mgp > 0
              ? "do zasilenia MAG"
              : p.ordered > 0
                ? "zam. u dostawcy: " + p.ordered
                : "strefa przyjęć pusta"}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        <div className="flex items-baseline justify-between gap-2">
          <div className="text-[11px] font-bold uppercase tracking-[0.1em] text-ink-mute">
            Lokalizacje{" "}
            <span className="font-normal normal-case tracking-normal">(pierwsza = pickingowa)</span>
          </div>
          <div className={cn("text-[10px] font-semibold tabular-nums", overLimit ? "text-destructive" : "text-ink-mute")}>
            {locStr.length}/{LOC_FIELD_LIMIT} zn.
          </div>
        </div>
        ...
        {chipMenu && (
          <div className="anim-popIn flex items-center gap-2 rounded-lg border bg-card px-2.5 py-2">
            <div className="flex-1 text-[13px]">
              Lokalizacja <b>{chipMenu}</b>
            </div>
            <Button variant="destructive" size="sm" onClick={() => removeChip(chipMenu)}>
              <Trash2 className="h-3.5 w-3.5" /> USUŃ
            </Button>
            <button onClick={closeChipMenu} className="px-1.5 text-ink-mute">
              <X className="h-4 w-4" />
            </button>
          </div>
        )}
