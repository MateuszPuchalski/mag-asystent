...
0.325.1 rozszerza bramkę WMS o pełny przebieg brakującej dostawy do wysyłki. Zmiana dotyczy testów i instrukcji, bez migracji ani nowych ustawień.
...
Odbiór seeded: wybrać wolne zadanie, odrzucić błędny bufor, zeskanować właściwy i przejść do części. Po ponowieniu wymagany jest świeży bufor.
...
Od 0.244.0 jest też `npm run sonda:zalacznik` (w `server/`, na serwerze — bierze
token i załącznik z bazy produkcyjnej). Wypisuje, którą drogą Allegro oddaje
plik: kody i typy, bez bajtów i bez adresów. Od 0.248.0 droga API idzie bez
nagłówka `Accept`, zgodnie ze specyfikacją; oczekiwany wiersz to „końcówka API
| bez Accept | 200 | image/jpeg", a dwie próby kontrolne z nagłówkiem mają 406.
