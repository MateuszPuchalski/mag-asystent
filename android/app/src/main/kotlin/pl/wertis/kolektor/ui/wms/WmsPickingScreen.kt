...
fun WmsPickingScreen(graph: AppGraph) {
    ...
    var scan by remember(view.generation, context) { mutableStateOf(WmsScanState(quantity = task?.remaining ?: 1)) }
    ...
    Column(
        ...
        val prompt = if (exception != null) "ZGŁOSZENIE — SKANUJ SKRZYNKĘ" else when (stage) {
        ...
        if (task != null && stage !in setOf(WmsStage.DONE, WmsStage.WAIT)) {
            ...
            if (exception == null) {
                Row(horizontalArrangement = Arrangement.spacedBy(10.dp), verticalAlignment = Alignment.CenterVertically) {
                    OutlineButton("−", modifier = Modifier.size(52.dp), enabled = scan.quantity > 1) { scan = scan.copy(quantity = scan.quantity - 1) }
                    Text("${scan.quantity} szt.", modifier = Modifier.weight(1f), fontWeight = FontWeight.ExtraBold, fontSize = 30.sp)
                    OutlineButton("+", modifier = Modifier.size(52.dp), enabled = scan.quantity < task.remaining) { scan = scan.copy(quantity = scan.quantity + 1) }
                }
                ...
                OutlineButton("ANULUJ ZGŁOSZENIE", modifier = Modifier.fillMaxWidth()) { exception = null }
            }
        } else when (stage) {
            ...
            WmsStage.DONE -> PrimaryButton("KOLEJNY WÓZEK", modifier = Modifier.fillMaxWidth()) {
        ...
        if (run != null && stage != WmsStage.DONE) {
            OutlineButton(if (tools) "ZAMKNIJ OPCJE" else "PROBLEM / ODŚWIEŻ", modifier = Modifier.fillMaxWidth()) { tools = !tools }
            if (tools) {
