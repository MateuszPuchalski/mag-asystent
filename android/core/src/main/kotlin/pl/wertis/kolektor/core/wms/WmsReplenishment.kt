...
enum class WmsReplenishmentStage { SOURCE, PRODUCT, QUANTITY, TARGET, BLOCKED, OTHER, DONE }
...
fun replenishmentCode(stage: WmsReplenishmentStage?, input: Scan): String =
    if (stage in setOf(WmsReplenishmentStage.SOURCE, WmsReplenishmentStage.TARGET) && input.kind == ScanKind.LOC) input.code else input.rawCode
