...
/* ── Wczytanie wertis.env — LUSTRO server/src/env-file.ts ────────────────────
   Konfiguracja ma JEDNO źródło: plik wertis.env czytany przez wszystkie trzy
   procesy (api, worker, sfera). Rozjazd między procesami to najgroźniejsza
    ...
    public static EnvFile Load()
    {
        ...
        var kandydaci = !string.IsNullOrEmpty(jawny)
            ...
            : new[]
            {
                ...
                // instalator kładzie plik w korzeniu C:\wertis, exe w C:\wertis\sfera-worker
                Path.Combine(AppContext.BaseDirectory, "..", "wertis.env"),
